...
#=======
from .models import InterviewGroup, Interview, Excerpt, ConceptExcerpt, TopicTag, get_concept_list
...
class ConceptExcerptEditForm(forms.ModelForm):
    add_topic_tags = MultiTagField(required=False,
                                 widget=forms.Textarea(attrs={
                                 'placeholder':"Add additional topic tags here. \nseperate with semicolons... \n\nTag 1;\nTag 2"}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        for tag in self.instance.topictag_set.all():
            ...
            self.fields["tag_%d" % tag.id] = forms.BooleanField(label=_(label),
                                                                   required=False,
                                                                   initial=False)          
    ...
    def save(self):
        ...
        for tag in self.instance.topictag_set.all():
            if self.cleaned_data.get("tag_%d" % tag.id):
                tag.excerpts.remove(self.instance)

                
        i = self.instance.interview # the interview
